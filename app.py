...
if option=='API認証とデータ取得':
    ...
    if uploaded_file is not None:
        ...
        st.write('認証が完了しました')
        user_name = st.text_input('ユーザー名を入力して下さい。', 'badassceo')
        user_id = client.get_user(username='badassceo').data.id
        ...
        if st.button('データ取得'):
            ...
            message.success('csvの出力が完了しました')
            st.download_button(
                label='csvファイルをダウンロード',
                data=csv,
                file_name='twitter_data_csv',
                mime='text/csv'
            )    
...
if option=='データ分析':
    uploaded_file = st.file_uploader('認証JSONファイルをアップロード', type=['csv']) 
    if uploaded_file is not None:
        df = pd.read_csv(uploaded_file)
        hist1 = alt.Chart(df, title='いいね数の傾向').mark_bar().encode(
            alt.X('いいね', bin=alt.Bin(extent  =[0, df['いいね'].max()], step=10000), title='いいね数'),
        ...
        result.reset_index(inplace=True)#inplace=Trueで上書き
        ...
        df.loc[df['いいね'] >= 50000, 'いいね評価'] = 'A'
        df.loc[(df['いいね'] < 50000) & (df['いいね'] >= 30000),'いいね評価'] = 'B'
        df.loc[(df['いいね'] < 30000) & (df['いいね'] >= 10000),'いいね評価'] = 'C'
        df.loc[(df['いいね'] < 10000) & (df['いいね'] >= 5000),'いいね評価'] = 'D'
        df.loc[df['いいね'] < 5000, 'いいね評価'] = 'E'
        
        df['文字数'] = df['ツイート本文'].str.len()
        grouped_fav = df.groupby('いいね評価')  
        mean_word = grouped_fav.mean()[['文字数']]
        mean_word.reset_index(inplace=True)
        mean_word.columns = ['等級', '平均文字数']
        
        hist2 = alt.Chart(mean_word, title='等級と文字数の関係性').mark_bar().encode(
        x='等級',
        y='平均文字数',
        tooltip=['平均文字数']
        )
